...
class EditarListaController {
  ...
  // Adicionando uma lista para armazenar itens temporários
  List<int> itensAdicionadosTemporarios = [];
  ...
  Future<void> adicionarItem(BuildContext context) async {
    ...
    if (nome.isNotEmpty && unidadeSelecionada != null) {
      ...
      if (existingListaItem.isEmpty) {
        ...
        // Armazena o idItem para possível remoção no cancelamento
        itensAdicionadosTemporarios.add(idItem);
      } else {
        await db.update(
          'ListaItens',
  ...
  void cancelarLista(BuildContext context) async {
    // Remove os itens adicionados temporariamente
    for (var idItem in itensAdicionadosTemporarios) {
      await db.delete(
        'ListaItens',
        where: 'id_item_fk = ? AND id_lista_fk = ?',
        whereArgs: [idItem, idLista],
    ...
    // Limpa a lista de itens adicionados temporariamente
    itensAdicionadosTemporarios.clear();
